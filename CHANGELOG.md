## Unreleased (toni-experience-software fork)

Carried on top of upstream 0.6.0 for the Scout show-car central display, where
the map renderer (Mapbox GL JS) runs in an offscreen CEF browser on a
touchscreen. Each commit is self-contained so it can be rebased onto a future
upstream release or sent as a PR.
